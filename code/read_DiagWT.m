...
for i=1:1:total_rdslice_WT
        ind=fileind_WT(i);
        disp(['Processing ',num2str(i/total_rdslice_WT*100,'%.4f'),'%, f_ind:',num2str(ind),' / ',num2str(fileind_WT(end))]);
...
%         disp(['No. ',num2str(i,'%06d'),', file_ind: ',num2str(ind,'%010d')])
        ...
        Tvol=T.*vol_gridFac;
        Tzmean=nanmean(nanmean(T.*hFacCmsk,1));Tzmean=reshape(Tzmean,[1,nz]);
        Tvolmean(i)=nansum(nansum(nansum(Tvol)))/Vol;
        Ld_mean(i)=get_Lrho_T(Tzmean,reshape(drF,[1,nz]),f0);
        ...
        Tsouth=reshape(Tsouth,[nx,nz]);
        Tcentr=reshape(Tcentr,[nx,nz]);
        Tnorth=reshape(Tnorth,[nx,nz]);    
        ...
        zF_S=reshape(zFac(:,wallind_S,:),[nx,nz]);   dzF_south=reshape(dzF(:,wallind_S,:),[nx,nz-1]);
        zF_C=reshape(zFac(:,wallind_C,:),[nx,nz]);   dzF_centr=reshape(dzF(:,wallind_S,:),[nx,nz-1]);
        zF_N=reshape(zFac(:,wallind_N,:),[nx,nz]);   dzF_north=reshape(dzF(:,wallind_S,:),[nx,nz-1]);
